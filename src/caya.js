 ...
 * @returns {number}
 */
caya.clamp = function(number, min, max) {
	if (number <= min)
		return min;
	else if (number >= max)
		return max;
 ...
 */
caya.iter = function(object, callback) {
 ...
 * @returns {string}
 */
caya.getFilenameExtension = function(filename) {
 ...
 *   the grid on clear.
 */
caya.Grid2D = function(w, h, defaultValue) {
	this.width = w;
	this.height = h;
 ...
 *
 * @class caya.AssetLoader
 * @classdesc Preloads and stores game assets.
 *
 */
caya.AssetLoader = function() {
	var self = this;
	var bank = {};
	 ...
	 * var loader = new caya.AssetLoader();
	 ...
	 *    }
	 * };
	 * var assetLoader = new caya.AssetLoader();
	 * assetLoader.load({
	 *    assets: assetList,
	 *    done: function() {
	...
	this.load = function(options) {
		...
		var n_toload = 0;
		var n_categories = 0;
		caya.iter(assetList, function(category, itemList) {
			n_categories++;
			caya.iter(itemList, function() {
				n_toload++;
			});
		});
		...
		caya.iter(assetList, function(category, itemList) {
			...
			if (loadHandler instanceof Function) {
				caya.iter(itemList, function(key, filename) {
	...
	//	var assetList = bank[category];
	//	return new FromObject(assetList);
	//};

	//function FromObject(assetList) {
	//	this.get = function(/**/) {
	//	};
	//}
	...
	this.acquire = function(object, keys) {
		caya.iter(keys, function(key, item) {
 ...
 *     is not found.
 */
caya.Configuration = function(key, defaultConfiguration) {
	var self = this;
	/**
	 * Save configuration.
	 */
	...
	this.load = function() {
		...
		caya.iter(self, function(key) {
			delete self[key];
		});
		// populate config
		caya.iter(config, function(key, item) {
			self[key] = item;
		});
	};
};
 ...
 * new caya.Surface({ fromCanvas: 'myCanvas' });
 *
 * @example
 * // Create a new virtual Surface with size 200x100
 * new caya.Surface({ width: 200, height: 100 });
 */
caya.Surface = function(options) {
	options = options || {};
	 ...
	 *
	 * @name caya.Surface#render
	 * @type {caya.Render}
	 */
	this.render = new caya.Render(ctx);
...
caya.Game = function(options) {
	 ...
	 *
	 * @param {caya.State} next - State to switch to.
	 */
